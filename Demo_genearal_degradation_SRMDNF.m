...
if kerneltype == 1
    ...
    kernelwidth = 0.2; % from a range of [0.2, 2] for sf = 2, [0.2, 3] for sf = 3, and [0.2, 4] for sf = 4.
...
% 
%     kernelc = kernel(:,:,:,1);
%     kernelc = cat(3,kernelc,kernelc,kernelc);
%     kernelc = cat(4,kernelc,kernelc,kernelc);
% 
%     kernelc = single(kernelc);
% 
%     net.layers{end+1} = struct('type', 'conv', ...
%          'weights', {{kernelc,  zeros(3, 1, 'single')}}, ...
%         'stride', 1, ...
%         'learningRate',lr10, ...
%         'dilate',1, ...
...
%         %degpar = single(net.meta.P*newKernel(:));
%         degpar = newKernel;
