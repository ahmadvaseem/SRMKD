...
for i=1:n
    ...
    switch l.type
        ...
        case 'concat'
                sigmaMap  = bsxfun(@times,ones(size(res(i).x,1),size(res(i).x,2),1,size(res(i).x,4)),permute(degvectors,[3 4 1 2]));
        ...
        case 'topk'
            res(i+1).x = vl_nntopk(res(i).x, res(1).x,l.name, opts.NMax);
...
if doder
    ...
    for i=n:-1:backPropLim
        ...
        switch l.type
            ...
            case 'topk'
                res(i).dzdx = vl_nntopk(res(i).x, res(1).x, l.name, opts.NMax, res(i+1).dzdx) ;
