...
%         for i=1:size(X,4)
%              % X(:,:,:,i) = imfilter(X(:,:,:,i),double(kernel(:,:,:,i)),'replicate'); 
%         end
%         n = size(c,1)/size(X,1);
...
if nargin <= 2 || isempty(dzdy)
     t = (d.^2)/2;
     Y = sum(t(:))/size(X,4); 
      %Y = (1-X)*(v-c);  %Y = (1-finalScore)*(valScore-gtScore);
...
%   dzdy = [] ;
% end
% %++++++++++++++++++++++++++++++++++++++++++++++++++++ when x and c data types are not same
% ax = class(x);
% ac = class(c);
% if ~strcmp(ax,ac)
%     x = single(x);
...
%             elseif(size(c,1) < size(x,1))
%                 x = X([1:end-1],:,:);
%             elseif(size(c,2) > size(x,2))
