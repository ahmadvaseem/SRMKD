...
for n_set = 1 : numel(setTest)
    ...
    for i = 1 : length(filepaths)
        ...
        for kx=1:kernel_size
            ...
            label_RGB = HR;
            blury_HR = imfilter(im2double(HR),double(k1),'replicate'); % blur%imfilter(im2double(HR),double(kernel),'replicate'); % blur
            ...
            %         score0 = Friquee_Score(label_RGB);
            %         score1 = Friquee_Score(im2uint8(LR_noisy));
            %         score2 = Friquee_Score(im2uint8(output_RGB));
% %              iqa_HR = Iqa_release(label_RGB);
% %              iqa_LR = Iqa_release(im2uint8(LR_noisy));
% %              iqa_SR = Iqa_release(im2uint8(output_RGB));
% %              disp('iqa_LR:');disp(iqa_LR); disp('iqa_HR:');disp(iqa_HR); disp('iqa_SR:');disp(iqa_SR);
            
%             fHR = rgb2ycbcr(im2double(label_RGB));
%             fHR = fHR(:,:,1);
%             fLR = rgb2ycbcr(LR_noisy);
%             fLR = fLR(:,:,1);
...
%             [PSNR_CurLR,SSIM_CurLR] = Cal_PSNRSSIM(label*255,fLR*255,sf,sf); %%% single
            ...
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% %             ssim_results(1,kx)  = SSIM_Cur; % ssim_results(i,kx)  = SSIM_Cur;
% %             iqaLR_results(1,kx) = iqa_LR;   %iqaLR_results(i,kx) = iqa_LR;
% %             iqaSR_results(1,kx) = iqa_SR;   %iqaSR_results(i,kx) = iqa_SR;
            %iqaSR_results_temp(1, kx)  = iqa_SR;
            %ssim_results_temp(1,kx) = SSIM_Cur;
            
        end
% %         ssim_Set{end+1} = single(reshape(ssim_results,1,1,kernel_size));
% %         iqaLR_Set{end+1} = single(reshape(iqaLR_results,1,1,kernel_size));
% %         iqaSR_Set{end+1} = single(reshape(iqaSR_results,1,1,kernel_size));
%         
%         iqaSR_results_temp = single(reshape(iqaSR_results_temp,1,1,kernel_size));

%        sub_model = Selection_Network(iqaSR_results_temp,ssim_results_temp, kernel_size);
%         iqaSR_results_temp = [];
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%---------------------
    end
% %     folderResult = 'ValidationSet';
% %     if ~exist(folderResult,'file')
% %         mkdir(folderResult); end
% %       save(fullfile(folderResult,['ssim_results_SR_',setTestCur]),'ssim_Set');
% %     save(fullfile(folderResult,['iqaLR_results_LR_',setTestCur]),'iqaLR_Set');
% %     save(fullfile(folderResult,['iqaSR_results_SR_',setTestCur]),'iqaSR_Set');
