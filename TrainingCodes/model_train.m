...
%% Plotting Graph
infr.state = 'firststart';
plotTrainingLoss(infr);
%%
%-------------------------------------------------------------------------
% Train and Test
%-------------------------------------------------------------------------
...
if start >= 1
    ...
    [net,state] = load(modelPath(start), 'net') ;
...
for epoch = start+1 : opts.numEpochs
    ...
    if  mod(epoch,5)~=1 && isfield(imdb,'set') ~= 0
        ...
        clear imdb;
        %[imdb] = generatepatches;          %% for Model Training
        [imdb] = SN_GeneratePatches;        %% for Selection Network Training
    end
 ...
 for i = 1:numel(state.lossArr)
   j = j+1;
   infr.state = 'iterations';
   infr.TrainingLoss = double(state.lossArr(i));
   infr.Iteration = j;
   plotTrainingLoss(infr);
 end
    state.lossArr = [];
...
%    if numel(opts.gpus) <= 1
%          [net, state] = process_epoch(net, state, imdb, opts, 'train');%[net, state] = processEpoch(net, state, opts, 'train') ;
%          %[net, state] = process_epoch(net, state, imdb, opts, 'val');%[net, state] = processEpoch(net, state, opts, 'val') ;
%         if ~evaluateMode
...
inputs = imdb.LRlabels(:,:,:,batch);
labels = imdb.HRlabels(:,:,:,batch);

% 
% kernel = imdb.kernels(:,:,batch);     %% Comment to train Selection Network

% n = randi(8);
% inputs = data_augmentation(inputs,n);
% labels = data_augmentation(labels,n);
% kernel = data_augmentation(kernel,n);

%% Comment to train Selection Network, Uncomment to Train model
% kk = P*reshape(kernel,size(P,2),size(kernel,3)); 
% ss = imdb.sigmas(batch);
% degvectors  = [kk;ss];

 function plotTrainingLoss(info)
 persistent plotObj
     if info.state == 'firststart'
        plotObj = animatedline('Color','r');
        xlabel('Iteration')
        ylabel('Loss')
        title('Training loss evolution')
     elseif info.state == 'iterations'
