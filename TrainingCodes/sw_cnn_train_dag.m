function [net,stats] = sw_cnn_train_dag(net, imdb, getBatch, varargin)
...
opts.model_name  = '';
opts.num_train_batch = 100;
opts.num_valid_batch = 0;
...
for epoch=start+1:opts.numEpochs
  ...
  rng(epoch + opts.randomSeed, 'twister') ;
  ...
  if numel(opts.gpus) <= 1
    ...
    spmd
      [net, state] = processEpoch(net, state, params,imdb, 'train') ;
      [net, state] = processEpoch(net, state, params,imdb, 'val') ;
...
for t=1:params.batchSize:numel(subset)
  ...
  for s=1:params.numSubBatches
    ...
    %inputs = params.getBatch(params.imdb, batch, net,imdb, mode) ;
